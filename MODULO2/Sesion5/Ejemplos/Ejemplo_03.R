...
# 1. Observemos algunas características del data frame Default del paquete ISLR,
...
# 2. Usando ggplot del paquete ggplot2, realicemos un gráfico de dispersión con 
#    la variable balance en el eje x, la variable income en el eje y, 
#    diferenciando las distintas categorías en la variable default usando el
#    argumento colour. Lo anterior para estudiantes y no estudiantes usando facet_wrap.

ggplot(Default, aes(x = balance, y = income, colour = default)) + 
  geom_point() + facet_wrap('student') + 
  theme_grey() + ggtitle("Datos Default")

# 3. Generemos un vector de índices llamado train, tomando de manera aleatoria
#    5000 números de los primeros 10,000 números naturales, esto servirá para 
#    filtrar el conjunto de entrenamiento y el conjunto de prueba del data frame
#    Default. Realicemos el gráfico de dispersión análogo al punto 2, pero para 
...
# 4. Ahora utilicemos la función tune junto con la función svm para seleccionar 
#    el mejor modelo de un conjunto de modelos, los modelos considerados serán 
#    aquellos obtenidos al variar los valores de los parámetros cost y gamma 
#    (usaremos un kernel radial).
...
# 6. Ajustemos nuevamente el mejor modelo, pero ahora con el argumento 
#    decision.values = TRUE. Obtengamos los valores predichos para el conjunto
#    de prueba utilizando el mejor modelo, las funciones predict, attributes y 
#    el argumento decision.values = TRUE dentro de predict.
...
# 7. Realicemos clasificación de las observaciones del conjunto de prueba
#    utilizando los valores predichos por el modelo y un umbral de decisión 
#    igual a cero. También obtengamos la matriz de confusión y proporciones como
