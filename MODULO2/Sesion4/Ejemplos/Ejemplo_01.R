...
# En `R` para calcular valores de las funciones de probabilidad, distribuci贸n 
# o cuantiles de la distribuci贸n binomial (discreta), usamos las funciones dbinom,
# pbinom y  qbinom respectivamente. Para generar muestras aleatorias de esta
# distribucion utilizamos la funcion rbinom.

# Consideremos un experimento binomial con n = 30 pruebas 
# identicas e independientes, en donde la probabilidad de exito en cada prueba
# es p = 0.2 (parametros n = 30 y p = 0.2)

#### Funcion de probabilidad

# Para obtener P(X = 20), es decir, la probabilidad de observar
# 20 exitos exactamente, en `R` ejecutamos
...
# Para obtener P(X <= 20), es decir, la probabilidad de observar
# a lo mas 20 exitos, en `R` corremos

pbinom(q = 20, size = 30, prob = 0.2)

# Para encontrar el valor mas peque駉 b tal que P(X <= b) >= 0.35,
...
# Para obtener una muestra aleatoria de tama駉 n = 1000, de la
# distribucion binomial con parametros como especificamos,
# hacemos
...
# Tambi茅n podemos observar las frecuencias relativas

(df1 <- as.data.frame(table(muestra)/length(muestra)))

#valores generados
valg <- as.character(df1$muestra) # distintos valores generados por rbinom
(valg <- as.numeric(valg)) # Convertimos a numeros

# Las frecuencias relativas son muy parecidas a las siguientes probabilidades

(v1 <- round(sapply(valg, dbinom, size = 30, p = 0.2), 3)) #el 3 es el # de decimales

# Combinamos df1 y v1 en un 煤nico data frame
...
(df2 <- melt(df2)) # funcion del paquete reshape2
...
# En `R` para calcular valores de las funciones de densidad, distribucion 
# o cuantiles de la distribucion normal (continua), usamos las funciones dnorm,
# pnorm y  qnorm respectivamente. Para generar muestras aleatorias de esta
# distribucion utilizamos la funcion rnorm.

# Consideremos una variable aleatoria (v.a.) X que se distribuye como normal
# con media 175 y desviacion estandar 6 (parametros mu = 175 y sigma = 6)

#### Funcion de densidad

x <- seq(-4, 4, 0.01)*6 + 175 # Algunos posibles valores que puede tomar la v.a. X (minimo: mu-4sigma, maximo: mu+4sigma)
y <- dnorm(x, mean = 175, sd = 6) # Valores correspondientes de la funcion de densidad de probabilidad
...
pnorm(q = 180, mean = 175, sd = 6) #sd desviacion estandar

par(mfrow = c(2, 2))
# Observemos la region que corresponde a esta probabilidad en la siguiente grafica en color rojo
...
# Observemos la region que corresponde a esta probabilidad en la siguiente grafica en color amarillo
...
# Observemos la region que corresponde a esta probabilidad en la siguiente grafica en color verde
...
# Observemos la region que corresponde a esta probabilidad en la siguiente grafica en color azul
...
dev.off() # Para mostrar solo una gr谩fica

#### Cuantiles

# Para encontrar el numero b, tal que P(X <= b) = 0.75, es decir,
...
# El cuantil se encuentra en el eje de medicion (eje horizontal)
...
# Para generar una muestra aleatoria de tama駉 n = 1000 de la v.a. X
# corremos la siguiente instrucci贸n
...
title(main = "Densidad de Probabilidad Normal", sub = expression(paste("Regla Empirica con ", mu == 175, " y ", sigma == 6)))
...
# En `R` para calcular valores de las funciones de densidad, distribucion 
# o cuantiles de la distribucion t de Student (continua), usamos las funciones dt,
# pt y  qt respectivamente. Para generar muestras aleatorias de esta
# distribucion utilizamos la funcion rt.

# Consideremos una variable aleatoria (v.a.) T que se distribuye como t
# de Student con 7 grados de libertad (gl) (parametro gl = 7)
...
# Para encontrar P(T <= 1.5), ejecutamos la siguiente instruccion

pt(q = 1.5, df = 7)

# Observemos la region que corresponde a esta probabilidad en la siguiente grafica
...
# Observemos la region que corresponde a esta probabilidad en la siguiente gr谩fica
...
# Para encontrar el numero d tal que P(T <= d) = 0.025, es decir, 
# el cuantil de orden 0.025, corremos la siguiente instruccion
...
# Mostramos el cuantil contrado en el eje de medici贸n (eje horizonta)
...
# Para generar una muestra aleatoria de tama帽o n = 1000 de la v.a. T
# corremos la siguiente instrucci贸n
