# EJEMPLO 4. Contraste de hipotesis
...
set.seed(104)
muestra <- rexp(n = 40, rate = 6) # media = 1/6 aprox 0.1667 (media de la poblacion)
tail(as.data.frame(muestra))
# estamos interesados en contrastar las hipotesis H0: mu = 0.1 vs H1: mu > 0.1 (contraste de cola superior)

# El valor observado del estadistico de prueba en este caso esta dado por

z0 <- (mean(muestra)-0.1)/(sd(muestra)/sqrt(40)) 
z0
# que proviene de una distribucion normal estandar aproximadamente.

# Supongamos que estamos interesados en encontrar la region de rechazo (de cola superior) 
# con un nivel de significancia alpha = 0.05, debemos encontrar el valor z_{0.05} 
# que satisface P(Z > z_{0.05}) = 0.05.
...
tail(as.data.frame(muestra))
# estamos interesados en contrastar las hipotesis H0: p = 0.9 vs H1: p < 0.9 (contraste de cola inferior)
