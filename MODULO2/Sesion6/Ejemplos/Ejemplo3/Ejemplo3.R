# Ejemplo 3. Modelos no estacionarios y prediccion

# https://github.com/AtefOuni/ts/tree/master/Data
    
# Serie de Produccion de Electricidad de Australia
CBE <- read.csv("cbe.csv", header = TRUE)
head(CBE)
Elec.ts <- ts(CBE[, 3], start = 1958, freq = 12)
plot(Elec.ts, xlab = "", ylab = "")
title(main = "Serie de Produccion de Electricidad Australiana",
      ylab = "Produccion de electricidad (GWh)",
      xlab = "Tiempo")

###

plot(diff(Elec.ts), xlab = "", ylab = "")
title(main = "Serie Diferenciada de Produccion de Electricidad Australiana",
      xlab = "Tiempo", ylab = "Dif Serie",
      sub = "Grafica de la serie diferenciada de primer orden")

###

plot(diff(log(Elec.ts)), xlab = "", ylab = "")
title(main = "Serie de log dif de Produccion de Electricidad Australiana",
      xlab = "Tiempo", ylab = "Dif log-Serie",
      sub = "Grafica de la serie log-transformada diferenciada de primer orden")

####################################################################################################################################################

                      # Simulacion y ajuste

# A continuacion, simulamos datos de un modelo ARIMA(1, 1, 1) y luego ajustamos un modelo a la serie simulada 
# para recuperar los parametros estimados.
...
###
      ...
      # Simulacion con la funcion arima.sim
...
####################################################################################################################################################
    ...
    # Serie de produccion de cerveza

CBE <- read.csv("cbe.csv", header = TRUE)
Beer.ts <- ts(CBE[, 2], start = 1958, freq = 12)
plot(Beer.ts, xlab = "", ylab = "")
title(main = "Serie de Produccion de Cerveza en Australia",
      ylab = "Produccion de Cerveza (Megalitros)",
      xlab = "Mes")
...
title(main = "Log de Serie de Produccion de Electricidad Australiana",
      ylab = "Log de Produccion de electricidad (GWh)",
      xlab = "Tiempo")
...
# Funcion para buscar un buen modelo
...
# Nuevo ajuste a los datos de la serie transformada de produccion 
# de electricidad
...
title(main = "Prediccion para la serie de produccion de electricidad",
      xlab = "Mes",
      ylab = "Produccion de electricidad (GWh)")
