# Al inicio es posible que no comprendas todo el codigo, trata de leerlo e ir 
# asimilando que es lo que realiza cada linea. Comenzaremos descargando los
# archivos CSV, con los ultimos resultados de los datos de la enfermedad COVID-19
...
# Tambien se puede hacer la lectura directamente desde el URL si asi se prefiere 
# (ten en cuenta que es un poco mas lento debido al tama�o de los archivos)
...
Sconf <- select(Sconf, Country.Region, Date, Value) # Pais, 
...
# Seleccionamos pais, fecha y acumulado de muertos
...
mex <- filter(mex, Infectados != 0) # Comienzan los infectados en Mexico
...
getwd()
setwd("C:/Users/monts/SantanderBEDU/MODULO2/Sesion3/Ejemplos")  #Fijando el wd
...
# A continuacion, te presentamos un panorama de la situacion que se ha estado 
# viviendo en Mexico, debido al coronavirus. Es informacion simple, que puede 
# resultar valiosa para algunas personas. Las graficas, las hemos realizado
...
p <- ggplot(mex, aes(x=Date, y=Infectados)) + 
  ...
  labs(x = "Fecha", 
       ...
       title = paste("Confirmados de COVID-19 en Mexico:", 
                     format(Sys.time(), 
...
p <- p +
  ...
  y = max(mex$Infectados), colour = "blue", size = 5, label = paste("Ultima actualizacion: ", mex$Infectados[dim(mex)[1]]))
p
...
p <- ggplot(mex, aes(x=Date, y=NI)) + 
  ...
  labs(x = "Fecha", y = "Incidencia (Numero de casos nuevos)",
       title = paste("Casos de Incidencia de COVID-19 en Mexico:", 
                     format(Sys.time(), 
                            tz="America/Mexico_City", usetz=TRUE))) +
  theme(plot.title = element_text(size=12))  +
  theme(axis.text.x = element_text(face = "bold", color="#993333" , size = 10, angle = 45, hjust = 1),
        axis.text.y = element_text(face = "bold", color="#993333" , size = 10, angle = 45, hjust = 1))  # color, angulo y estilo de las abcisas y ordenadas
...
p <- p +
  ...
  annotate("text", x = mex$Date[round(dim(mex)[1]*0.4)], y = max(mex$NI), colour = "blue", size = 5, 
           label = paste("Ultima actualizacion: ", mex$NI[length(mex$NI)]))
p
...
p <- ggplot(mexm, aes(x=Date, y=Muertos)) + geom_line( color="red") + 
  ...
  labs(x = "Fecha", 
       ...
       title = paste("Muertes por COVID-19 en Mexico:", format(Sys.time(), tz="America/Mexico_City",usetz=TRUE))) +
  theme(axis.text.x = element_text(face = "bold", color="#993333" , size = 10, angle = 45, hjust = 1),
        axis.text.y = element_text(face = "bold", color="#993333" , size = 10, angle = 45, hjust = 1))  # color, angulo y estilo de las abcisas y ordenadas
...
p <- p +
  ...
  annotate("text", x = mexm$Date[round(dim(mexm)[1]*0.4)], 
           y = max(mexm$Muertos), colour = "red", size = 5, label = paste("Ultima actualizacion: ", mexm$Muertos[dim(mexm)[1]]))
p

# Muertes por Dia
p <- ggplot(mexm, aes(x=Date, y=NM)) + 
  geom_line(stat = "identity") + 
  labs(x = "Fecha", y = "Numero de nuevos decesos",
       title = paste("Nuevos decesos por COVID-19 en Mexico:", 
...
p <- p +
  ...
  annotate("text", x = mexm$Date[round(dim(mexm)[1]*0.2)], 
           y = max(mexm$NM), colour = "red", size = 5, label = paste("Ultima actualizacion: ", mexm$NM[dim(mexm)[1]]))
p
...
p <- ggplot(mex, aes(x=Date, y=Infectados)) + geom_line(color="blue") + 
  labs(x = "Fecha", 
       ...
       title = paste("COVID-19 en Mexico:", format(Sys.time(), tz="America/Mexico_City",usetz=TRUE))) +
...
p <- p +
  theme(plot.margin=margin(10,10,20,10), plot.caption=element_text(hjust=1.05, size=10)) +
  annotate("text", x = mex$Date[round(dim(mex)[1]*0.4)], 
           y = max(mex$Infectados), colour = "blue", size = 5, label = paste("Ultima actualizacion para Infectados:", mex$Infectados[dim(mex)[1]])) +
  annotate("text", x = mex$Date[round(dim(mex)[1]*0.4)], 
           y = max(mex$Infectados)-100000, colour = "red", size = 5, label = paste("Ultima actualizacion para Muertes:", mex$Muertos[dim(mex)[1]])) 
p

# Tasa de Letalidad: La tasa de letalidad observada para un dia determinado, la
# calculamos dividiendo las muertes acumuladas reportadas hasta ese dia, entre
# el acumulado de casos confirmados para el mismo dia. Multiplicamos el resultado
# por 100 para reportarlo en forma de porcentaje. Lo que obtenemos es el 
# porcentaje de muertes del total de casos confirmados.
...
p <- p +
  ...
  annotate("text", x = mexm$Date[round(length(mexm$Date)*0.2)], 
           y = max(mexm$Letalidad)-1, colour = "red", size = 4, label = paste("ultima actualización: ", mexm$Letalidad[dim(mexm)[1]], "%", sep = "")) 
p

# Factores de Crecimiento:

# El factor de crecimiento de infectados para un dia determinado, lo calculamos 
# al dividir el acumulado de infectados para ese dia, entre el acumulado de 
# infectados del dia anterior. El factor de crecimiento de muertes lo calculamos 
# de forma similar.
...
p <- p +
  annotate("text", x = mex$Date[round(length(mex$Date)*0.4)], y = max(mex$FCM), colour = "blue", size = 5, label = paste("Ultima actualizacion para infectados: ", round(mex$FCI[dim(mex)[1]], 4))) +
  annotate("text", x = mex$Date[round(length(mex$Date)*0.4)], y = max(mex$FCM)-0.2, colour = "red", size = 5, label = paste("Ultima actualizacion para muertes: ", round(mex$FCM[dim(mex)[1]], 4))) 
p
